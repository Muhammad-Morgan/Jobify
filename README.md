...
- add middleware
- UPDATE: middleware is debrecated, and Next.js provides a codemod to migrate from middleware.ts to proxy.ts, by running: npx @next/codemod@canary middleware-to-proxy .
...
app/(dashboard)/jobs/page.tsx (lines 8-17) always prefetches ["jobs","","all",1], so navigating to /jobs?page=2&jobStatus=pending rehydrates with the wrong query key and immediately triggers a second fetch that shows the spinner. Pull searchParams into this server component (or fetch the jobs right here) so the dehydrated key/data matches whatever the client will request.
components/Jobs/JobsList.tsx (lines 18-31) calls the getAllJobsAction server action from the browser for every filter/page change, so each click hits Clerk → server action → Prisma over the network. Either move the list into a server component (no client fetch needed) or at least prefetch neighbor pages via queryClient.prefetchQuery/placeholderData and cache the last result so React Query doesn’t drop back to the loading state on each navigation.

---

lib/utils/actions.ts (lines 42-102) runs auth() and two sequential Prisma calls every time the list loads. Fetching the list and the count inside a $transaction (or Promise.all) and passing the userId down instead of re-reading it inside the action shaves off two round-trips for every pagination event. Also use .select to limit fields if the cards only need id, position, company, location, status, and createdAt.
prisma/schema.prisma (lines 15-26) defines Job with no indexes even though queries filter by clerkId, status, and order by createdAt. Adding indexes (e.g. @@index([clerkId, createdAt]) plus ones on status or position) lets PostgreSQL satisfy findMany/count without scanning each user’s entire job history.

---

components/Jobs/SearchForm.tsx (lines 21-33) and components/Jobs/ComplexButtonContainer.tsx (lines 30-40) use imperative router.push without a transition, so every change blocks the UI and re-renders the whole route even though only the query string changes. Wrap these pushes in startTransition, use router.replace for pagination, and debounce the search form so the list isn’t reloading mid-typing.
lib/utils/db.ts (lines 1-20) spins up a dedicated pg Pool per lambda. On Vercel this quickly exhausts Postgres connections, forcing Prisma to wait before each query. Use Prisma Accelerate/Data Proxy or swap to @vercel/postgres so you reuse a platform-managed pool.

---

Chart/stat actions (lib/utils/actions.ts (lines 160-224)) currently re-run heavy aggregations on each navigation and even log intermediate results. Cache those queries separately with React Query and a generous staleTime, and drop the console.log so they don’t block the event loop.
Rendering-wise, components/Jobs/JobsList.tsx (lines 48-51) renders every job card at once. If users can have large histories, wrap the grid in a virtualized list (e.g. react-virtual) so changing pages only mounts ~10 cards instead of dozens, and consider eagerly prefetching /jobs?page=n±1 once the current page loads to keep navigation instant.
